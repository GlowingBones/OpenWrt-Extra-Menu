...
echo "Installing 3rd Party Menu Editor from $BASE_DIR"
...
cp "$BASE_DIR/thirdparty.lua" /usr/lib/lua/luci/controller/thirdparty.lua
cp "$BASE_DIR/embed.htm"      /usr/lib/lua/luci/view/thirdparty/embed.htm
cp "$BASE_DIR/medit.htm"      /usr/lib/lua/luci/view/thirdparty/medit.htm
...
if [ ! -f /etc/thirdparty_menu.conf ]; then
    cp "$BASE_DIR/thirdparty_menu.conf.example" /etc/thirdparty_menu.conf
...
echo "Done. Open LuCI and look for the '3rd Party' top menu, then 'Menu Edit'."
